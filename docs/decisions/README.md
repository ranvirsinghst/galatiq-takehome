...
| [008](#adr-008-catalog-price-validation-with-a-tolerance-band) | Catalog price validation with a tolerance band | Accepted; implemented, verification recorded |
...
## ADR-008: Catalog price validation with a tolerance band

**Status and context.** Accepted on 2026-09-08 from a design review pass; implemented on 2026-09-09. Actual verification and live observations are recorded in [delivery status](../build/status.md#catalog-price-validation-follow-up). The following context describes the pre-feature behavior. Validation confirmed that quantity times unit price equals the line total and that line amounts reconcile to the payable total, but it never compared a unit price against any reference. An inflated price that was internally consistent was therefore approved. `data/invoices/invoice_1010.txt` charges $300.00 for WidgetA on a line noted "(rush order)" against $250.00 everywhere else in the corpus, and `tests/fixtures/expected/outcomes.json` previously expected that invoice to be approved with no findings. Pricing is the one fraud vector named in the [assignment](../assignment.md) that the prototype cannot detect, and the brief explicitly invites a price table as an extension of the seed data.

**Decision.** Add a `prices` table to the committed [schema](../../invoice_agent/sql/schema.sql) and [seed](../../invoice_agent/sql/seed.sql), holding one USD unit price per stocked item: WidgetA 250.00, WidgetB 500.00, GadgetX 750.00. These are **derived from the supplied invoice corpus, not authoritative vendor contracts**, and the seed comment must say so. Expose a second typed tool, `lookup_price`, alongside `lookup_inventory`; the agent obtains prices through the tool rather than from prompt text. Compare each line's converted USD unit price against its catalog price and emit a deterministic finding when the deviation exceeds **10%**: above catalog is a `PRICE_OVERCHARGE` blocker, below catalog is a warning the VP must acknowledge. Items with no catalog row are not price-checked; they already reject on `UNKNOWN_ITEM` or `OUT_OF_STOCK` before pricing is reachable. Where a line carries note text such as "Volume discount", "Expedited", or "Rush", surface that note on the finding so the VP reviews the stated justification rather than the bare number.

**Alternatives and rationale.** An exact-match price check is not viable: `invoice_1014.xml` is denominated in EUR, and at the fixed policy rate of 1.10 its WidgetB line converts to $522.50 against a $500.00 catalog price, a 4.5% deviation on an invoice expected to be approved. Exact matching would reject it — a false positive on a legitimate invoice. The band must absorb that 4.5% while catching the 20% rush-order overcharge; no observed price falls between those figures, so 10% separates them with margin on both sides. Per-currency price rows were rejected as premature: exactly one non-USD invoice exists, and the tolerance already covers conversion and rounding noise. Progressive discount tiers were rejected because no supplied invoice uses tiered pricing, making the rule untestable against real data. Rejecting every below-catalog price was rejected because legitimate volume discounts exist in the corpus (1013 at −4% on two lines) and auto-rejecting them would substitute a mechanical rule for the judgment the VP review exists to demonstrate. Giving the agent raw SQL access instead of a typed tool was rejected outright: the agent's context is populated from untrusted invoice text, and arbitrary SQL against the inventory and payment ledger is an injection surface that buys no capability the typed tool lacks.

**Consequences and tradeoffs.** `invoice_1010.txt` is expected to flip from approved to rejected, and its entry in `outcomes.json` must be updated as part of the change; this is the only expected outcome that changes. The tool count rises from one to two, which remains within the minimal-toolset intent of [ADR-007](#adr-007-one-review-node-with-early-deterministic-rejection). A 10% band is a declared policy default, not a measured fraud threshold: a disciplined overcharge under 10% passes undetected, and this limitation belongs in the README's scope section alongside the existing statement that the system cannot prove fraud or verify vendor identity. Because the catalog is derived from the invoices it validates, it cannot detect a price inflated consistently across the whole corpus. Below-catalog warnings add VP review load on discounted invoices that would otherwise pass without comment. Price findings are deterministic and, consistent with [ADR-002](#adr-002-deterministic-payment-authority-and-usd-accounting), the model cannot waive them.

**Verification and evidence.** Implemented with run-bound typed catalog evidence and independent comparisons against SQLite facts before and inside the payment transaction. Regression tests cover exact and adjacent tolerance boundaries, EUR conversion, discount acknowledgment, catalog/tool errors, tampered evidence, changed catalog rows, rollback, and historical rendering. Offline evaluation passes all 23 cases with only invoice 1010's isolated expected decision changed. Real CLI runs reject 1010 with `PRICE_OVERCHARGE`, no VP calls and no payment; 1001 and 1014 still pay. Full command results, run paths, provider failures/retries and inspection limitations are in [delivery status](../build/status.md#catalog-price-validation-follow-up). The explicit missing-price and precision defaults are documented in the [implementation contract](../specs/spec-catalog-price-validation/implementation.md). Price sources remain in [design review action items](../action-items.md#price-evidence).

**Revisit when** authoritative vendor price lists or contract terms become available, per-vendor or per-currency pricing enters scope, measured false-positive or false-negative rates justify moving the 10% band, or discount legitimacy needs verification against purchase orders rather than invoice-stated notes.
