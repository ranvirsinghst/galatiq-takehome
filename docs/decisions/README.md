...
| [008](#adr-008-catalog-price-validation-with-a-tolerance-band) | Catalog price validation with a tolerance band | Accepted; implementation pending |
...
**Status and context.** Accepted on 2026-09-08 from a design review pass; implementation pending, so nothing below is verified behavior. Validation currently confirms that quantity times unit price equals the line total and that line amounts reconcile to the payable total, but it never compares a unit price against any reference. An inflated price that is internally consistent is therefore approved. `data/invoices/invoice_1010.txt` charges $300.00 for WidgetA on a line noted "(rush order)" against $250.00 everywhere else in the corpus, and `tests/fixtures/expected/outcomes.json` expects that invoice to be approved with no findings. Pricing is the one fraud vector named in the [assignment](../assignment.md) that the prototype cannot detect, and the brief explicitly invites a price table as an extension of the seed data.
...
**Verification and evidence.** Not yet implemented; no evidence exists. Required before this decision may be treated as delivered: a unit test that the +20% rush line on invoice 1010 produces `PRICE_OVERCHARGE`; a unit test that the +4.5% converted EUR line on invoice 1014 stays inside the band and remains approved; a unit test that a below-catalog line yields a warning rather than a blocker; a test that unknown items are not price-checked; a `lookup_price` tool-protocol test mirroring the existing inventory coverage checks in `tools.py`; and a full `scripts/evaluate.py` run showing invoice 1010 as the only changed outcome. Price sources for every seeded value are tabulated in [design review action items](../action-items.md#price-evidence).
